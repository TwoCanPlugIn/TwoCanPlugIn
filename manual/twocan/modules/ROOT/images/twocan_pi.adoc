...
Similarly the Mac OSX version uses four “baked in” classes, one
supporting the generic Log File reader, one supporting the Rusoku Toucan
adapter (see above), one supporting the Kvaser Leaflight adapter (also see above)
and the other supporting serial USB CAN bus adapters
...
Version 2 of the plugin also supports a bi-directional gateway that
converts NMEA 183 sentences recieved from OpenCPN to NMEA 2000 messages
and transmits them onto the NMEA 2000 network.
...
and Kvaser Leaflight devices the plugin also installs the necessary device
drivers, which will also detect the correct USB port.

The drivers for these two devices are based on the MacCAN API.
https://github.com/mac-can[Mac CAN API]
