...
void TwoCan::SetPluginMessage(wxString& message_id, wxString& message_body) {
	...
	else if (message_id == _T("TWOCAN_MEDIA_REQUEST")) {
	...
	// Allow any plugin to send NMEA 2000 frames onto the network
		...
		if ((deviceMode == TRUE) && (twoCanDevice != nullptr)) {
			...
			if (reader.Parse(message_body, &root) > 0) {
				...
				header.source = networkAddress; // root["source"].AsInt();
				...
				if (payload.size() > 8) {
					...
					returnCode = twoCanDevice->TransmitFrame(id, payload.data());
					if (returnCode != TWOCAN_RESULT_SUCCESS) {
						wxLogMessage("TwoCan Plugin, Error sending raw message: %d", returnCode);
					}
				}
	...
	// Handle TwoCan Autopilot Plugin dialog commands
		...
		if ((deviceMode == TRUE) && (autopilotModel != AUTOPILOT_MODEL::AUTOPILOT_NONE) && (twoCanDevice != nullptr) && (twoCanAutopilot != nullptr)) {
			std::vector<CanMessage> messages;
			unsigned int id;
			int returnCode;
...
bool TwoCan::LoadConfiguration(void) {
	...
	if (configSettings) {
		...
		autopilotModel = static_cast<AUTOPILOT_MODEL>(configSettings->ReadLong(_T("Autopilot"), AUTOPILOT_MODEL::AUTOPILOT_NONE));
	...
	if (twoCanDevice != nullptr) {
		if (twoCanDevice->IsRunning()) {
			...
			if (threadError == wxTHREAD_NO_ERROR) {
				...
				if ((deviceMode == TRUE) && (autopilotModel != AUTOPILOT_MODEL::AUTOPILOT_NONE)) {
