...
// 2.0 - 01-12-2020 Bi-Directional Gateway, Kvaser support on Mac OSX, Fast Message Assembly & SID generation fix 
...
TwoCan::TwoCan(void *ppimgr) : opencpn_plugin_116(ppimgr), wxEvtHandler() {
	...
	// Events generated by the Autopilot dialog
	...
	if (twoCanDevice != nullptr) {
		if (twoCanDevice->IsRunning()) {
			...
			if (threadError == wxTHREAD_NO_ERROR) {
				...
				// when runing on Windows, refer to https://forums.wxwidgets.org/viewtopic.php?t=13726. However no seems to be fixed
